#!/usr/bin/env python
"""
Test script to verify model optimization techniques for Phi-3 loading
"""
...
def test_model_optimizations():
    """Test the model optimization techniques"""
    print("🔍 Testing model optimization techniques for Phi-3 loading...")
    ...
    # Configuration for testing with optimizations
    config = {
        "llm": {
            "reasoning": {
    ...
    try:
        # Initialize the handler with optimizations
        print("🔄 Initializing DirectML handler with optimizations...")
        handler = DirectMLOptimizedHandler(config)
        print(f"✅ Handler initialized with device: {handler.device}")
        print(f"📊 KV Cache Enabled: {handler.kv_cache is not None}")
        
        # Test loading the Phi-3 model with optimizations
        print("📥 Loading Phi-3 model with optimizations...")
        ...
        try:
            ...
            if result:
                print("✅ Phi-3 model loaded successfully with optimizations!")
                ...
                print("❌ Failed to load Phi-3 model with optimizations")
                return False
...
if __name__ == "__main__":
    success = test_model_optimizations()
