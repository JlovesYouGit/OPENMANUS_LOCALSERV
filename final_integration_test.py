...
def final_integration_test():
    ...
    # Poll for result like the web UI does (but with our extended timeout)
    max_polls = 70  # Should be enough for 5+ minutes with 5-second intervals
    poll_count = 0
    response = None
    ...
    while poll_count < max_polls:
        ...
        # After 65 polls (about 5.5 minutes), manually trigger timeout check
        if poll_count == 65:
...
if __name__ == "__main__":
    ...
    response = final_integration_test()
    ...
    if response and response.get('status') == 'failed' and 'timeout' in response.get('error', '').lower():
        ...
        print("✅ Timeout handling is working end-to-end")
        print("✅ Frontend and backend are properly synchronized")
        print("✅ Users will now get accurate timeout error messages")
    else:
