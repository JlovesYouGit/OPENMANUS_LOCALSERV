...
class Config:
        ...
        if not self._initialized:
            with self._lock:
                if not self._initialized:
                    ...
                    # Initialize local mode attributes
                    self.is_local_mode = True  # Enable local mode by default
                    self.local_model_handler = None
                    self._initialize_local_model_handler()
    ...
    def llm(self) -> Dict[str, LLMSettings]:
        return self._config.llm if self._config else {}

    @property
    def sandbox(self) -> Optional[SandboxSettings]:
        return self._config.sandbox if self._config else None

    @property
    def daytona(self) -> Optional[DaytonaSettings]:
        return self._config.daytona_config if self._config else None

    @property
    def search_config(self) -> Optional[SearchSettings]:
        return self._config.search_config if self._config else None

    @property
    def mcp_config(self) -> Optional[MCPSettings]:
        """Get the MCP configuration"""
        return self._config.mcp_config if self._config else None

    @property
    def run_flow_config(self) -> Optional[RunflowSettings]:
        """Get the Run Flow configuration"""
        return self._config.run_flow_config if self._config else None
    ...
    def _initialize_local_model_handler(self):
        """Initialize local model handler if in local mode"""
        try:
            # Check if we should use local models
            if self.is_local_mode:
                # Try to use VLLM handler first for better performance
                try:
                    ...
                    if "llm" in config_data:
                        ...
                        if "reasoning" in config_data["llm"]:
                            config_data["llm"]["reasoning"]["model_path"] = "./models/qwen2-0.5b"  # Updated to use Qwen2-0.5B
                    
                    # Try to initialize the VLLM handler WITHOUT testing model loading during startup
                    vllm_handler = VLLMOptimizedHandler(config_data)
                    # Don't test model loading during startup to prevent hangs
                    self.local_model_handler = vllm_handler
                    print("🔄 VLLM optimized handler initialized (deferred model loading)")
                    return  # Successfully initialized VLLM handler
                except ImportError as vllm_error:
                    print(f"⚠️  VLLM not available: {vllm_error}")
                except Exception as vllm_error:
                    print(f"⚠️  VLLM initialization failed: {vllm_error}")
                    import traceback
                    traceback.print_exc()
                ...
                try:
                    ...
                    print("⚠️  DirectML fixed handler not available, trying standard DirectML handler...")
                    ...
                    try:
                        ...
                        return
                    except ImportError as directml_error:
                        print(f"⚠️  Standard DirectML handler not available: {directml_error}")
                    except Exception as directml_error:
                        print(f"⚠️  Standard DirectML handler initialization failed: {directml_error}")
                        import traceback
                        traceback.print_exc()
                
                # If all handlers fail, set to None
                print("⚠️  All local model handlers failed to initialize")
                self.local_model_handler = None
        except Exception as e:
            print(f"⚠️  Failed to initialize local model handler: {e}")
            import traceback
            traceback.print_exc()
            self.local_model_handler = None
